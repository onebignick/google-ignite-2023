...
def login():
    with sqlite3.connect("database.db") as con:
        cur = con.cursor()
        get_user_id_sql = (
            """SELECT User(user_id, user_password) FROM User WHERE User.user_name=?"""
        )
        result = {}
...
def api_home_page():
    with sqlite3.connect("database.db") as con:
        ...
        for i in range(len(result)):
            ...
            d["id"] = result[i][0]
            d["author"] = result[i][-1]
            d["authorId"] = result[i][1]
            d["title"] = result[i][2]
            d["subtitle"] = result[i][3]
            d["content"] = result[i][4]
            d["category"] = str(result[i][7])
            d["authorImageUrl"] = "123"
            d["imageUrl"] = result[i][-5]
            d["views"] = result[i][-4]
            d["likes"] = result[i][-3]
            d["createdAt"] = result[i][5]
            a.append(d)
        r = {}
        r["body"] = a
    return r
...
def api_create_post():
    if request.method == "POST":
        with sqlite3.connect("database.db") as con:
            post_type_id_sql = (
                """SELECT PostType(post_type_id) FROM PostType WHERE post_type=?"""
            )
            post_type_id = cur.execute(
                post_type_id_sql, request.json["post_type"]
            ).fetchone()[0]
            ...
            to_add = {
                ...
                "post_source": request.json["post_source"]
                if request.json["post_source"]
                else "",
                "post_read_time": request.json["post_read_time"]
                if request.json["post_read_time"]
                else "",
                "post_image_url": request.json["post_image_url"]
                if request.json["post_image_url"]
                else "",
            ...
            if post_type_id == 3:
                comment_id = cur.execute(
                    """SELECT seq FROM sqlite_sequence WHERE table_name=Post)"""
                ).fetchone()
...
def api_getBuzz():
    user_query = (
        f"""SELECT UserID FROM Users WHERE Username ='{request.json["username"]}'"""
    )
...
def get_centerType(centerType_id):
    centerType = cur.execute(
        "SELECT * FROM centreType WHERE center_type_id = ?", (centerType_id,)
    ).fetchone()
    return centerType
...
def createCenterType():
    if request.method == "POST":
        ...
        cur.execute(
            "INSERT INTO CenterType (center_type_id, center_type) VALUES (?, ?)",
...
def createAppointment():
    if request.method == "POST":
        ...
        cur.execute(
            "INSERT INTO Appointment (appointment_id, appointment_title, appointment_date) VALUES (?, ?, ?)",
...
def updateAppointment(appointment_id):
    ...
    if request.method == "POST":
        ...
        cur.execute(
            "UPDATE posts SET appointment_title = ?, appointment_date = ?"
